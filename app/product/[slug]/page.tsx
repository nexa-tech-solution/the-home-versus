...
import { ArrowLeft, Clock, User, Star, CheckCircle2, XCircle, AlertCircle, ShoppingCart, ArrowRight } from "lucide-react";
      ...
      <main className="container max-w-5xl py-8 md:py-20 px-4 md:px-0">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-xl text-foreground font-semibold italic mb-8 border-l-4 border-accent pl-6">
              "{product.highlight}"
            </p>
            ...
            <div className="flex items-center gap-6 mt-auto">
                <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">MSRP</p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
            <div className="rounded-3xl border border-border bg-card overflow-hidden shadow-sm">
                <div className="bg-pro/10 p-4 border-b border-border flex items-center gap-2">
                    ...
                    <h3 className="font-display font-bold text-pro uppercase tracking-widest text-sm">The Positives (Pros)</h3>
                </div>
            ...
            <div className="rounded-3xl border border-border bg-card overflow-hidden shadow-sm">
                <div className="bg-con/10 p-4 border-b border-border flex items-center gap-2">
                    ...
                    <h3 className="font-display font-bold text-con uppercase tracking-widest text-sm">Room for Improvement (Cons)</h3>
                </div>
                <ul className="p-8 space-y-4">
        ...
        <section className="mb-20">
            <h2 className="font-display text-3xl font-bold mb-8">Technical Specifications</h2>
            <div className="rounded-3xl border border-border bg-card overflow-hidden shadow-sm">
                <div className="divide-y divide-border">
                    {product.specs.map((spec, i) => (
                        <div key={i} className="grid grid-cols-2 hover:bg-secondary/5 transition-colors">
                    ...
                    {relatedComparisons.map((comp) => (
                        ...
                        >
                            <div className="w-20 h-20 shrink-0 bg-white rounded-2xl border border-border flex items-center justify-center p-2 group-hover:scale-105 transition-transform">
                                <img src={comp.imageA} alt="" className="max-w-full max-h-full object-contain" />
                            </div>
        ...
        {/* Related Products */}
        <section className="mb-20">
            <h2 className="font-display text-3xl font-bold mb-8">Other Recommended Products</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {product.relatedProducts.map((p) => (
                    <div key={p.slug} className="group relative bg-card rounded-3xl border border-border p-6 hover:shadow-2xl transition-all">
                        ...
                        <h3 className="font-display text-xl font-bold text-foreground mb-4">{p.name}</h3>
                        <div className="flex items-center justify-between mt-auto">
                            <span className="text-2xl font-bold text-accent">{p.price}</span>
                            <div className="flex items-center gap-2">
                                <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">See more</span>
