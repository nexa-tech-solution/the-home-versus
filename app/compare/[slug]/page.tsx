...
const articleData: Record<string, any> = {
  "dyson-v15-vs-shark-stratos": {
    ...
    specs: [
      { label: "Weight", valueA: "6.8 lbs", valueB: "8.9 lbs", winner: "A" as const },
      { label: "Runtime", valueA: "60 min", valueB: "50 min", winner: "A" as const },
      { label: "Noise Level", valueA: "Quiet enough (baby naps safe!)", valueB: "Moderate — might wake a light sleeper", winner: "A" as const },
      { label: "Pet Hair Pickup", valueA: "Excellent (laser reveals hidden hair)", valueB: "Very Good (anti-hair wrap)", winner: "A" as const },
      { label: "Easy to Clean", valueA: "Moderate", valueB: "Super easy — one-click empty", winner: "B" as const },
      { label: "Price", valueA: "$749.99", valueB: "$399.99", winner: "B" as const },
    ],
    prosA: [
    ...
    sections: [
      {
        ...
        content: "We unleashed two golden retrievers in a carpeted living room and waited 24 hours. The Dyson V15's laser literally showed us hair we couldn't see. Its motorbar head picked up 97% of embedded pet hair in one pass. The Shark Stratos did a solid job too — its anti-hair wrap brush roll means you'll never have to cut tangled hair off the roller. But in raw pickup performance, Dyson edges ahead.",
      },
      {
        title: "😴 Noise Level: Can You Vacuum During Nap Time?",
        content: "This is the question every parent asks. The Dyson V15 on Eco mode is surprisingly quiet — we ran it in the hallway while our 18-month-old napped, and she didn't stir. The Shark Stratos is louder, especially on max power. If nap-time cleaning is your thing, Dyson wins this round.",
      },
      {
        title: "💰 Value for Money: Is the Dyson Worth 2x the Price?",
        content: "Here's the honest truth: the Shark Stratos does about 85% of what the Dyson does for 53% of the price. If your budget is tight and you need a great cordless vacuum, the Shark is an absolute steal. But if you want the best of the best and that laser dust-reveal gives you joy, the Dyson is worth the splurge.",
      },
    ],
    verdict: {
      overallWinner: "Dyson V15 Detect",
      summary: "Both are excellent vacuums, but they serve different budgets and priorities. The Dyson V15 Detect wins on performance, quietness, and tech features. The Shark Stratos wins on value and ease of maintenance.",
      options: [
        {
          product: "Dyson V15 Detect",
          reason: "You have pets with heavy shedding, care about quiet operation during nap time, and don't mind paying premium for the best performance.",
          amazonUrl: "https://www.amazon.com/dp/B0CK2RJVXW",
        },
        {
          product: "Shark Stratos",
          reason: "You want excellent cleaning at half the price, prefer easy maintenance (one-click empty!), and live in a smaller home.",
          amazonUrl: "https://www.amazon.com/dp/B0BPM3KZLC",
        },
      ],
    },
...
export async function generateMetadata(
  ...
  return {
    ...
    keywords: [article.category, article.productA.name, article.productB.name, "comparison", "review"],
    openGraph: {
      title: article.title,
      description: article.intro,
...
export default async function ComparisonArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = articleData[slug];
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container py-20 text-center">
          <h1 className="font-display text-4xl font-bold text-foreground mb-6">Comparison Not Found</h1>
          <Link href="/" className="text-accent hover:underline inline-flex items-center gap-2 font-bold text-lg">
      ...
      <article className="container max-w-4xl py-12 md:py-20">
        ...
        <div className="mb-20 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <ProsConsList productName={article.productA.name} pros={article.prosA} cons={article.consA} />
            <ProsConsList productName={article.productB.name} pros={article.prosB} cons={article.consB} />
          </div>
        </div>
